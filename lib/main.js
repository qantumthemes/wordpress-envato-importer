...
$.qtEnvImp.expression1 = /http:\/\/themeforest\.net\//;
$.qtEnvImp.expression2 = /http:\/\/codecanyon\.net\//;
$.qtEnvImp.expression3 = /http:\/\/videohive\.net\//;
$.qtEnvImp.expression4 = /http:\/\/audiojungle\.net\//;
$.qtEnvImp.expression5 = /http:\/\/graphicriver\.net\//;
$.qtEnvImp.expression6 = /http:\/\/photodune\.net\//;
$.qtEnvImp.expression7 = /http:\/\/3docean\.net\//;
$.qtEnvImp.expression8 = /http:\/\/activeden\.net\//;


$.qtEnvImp.singleItem = /\/item\//;

$.qtEnvImp.apiUrl = "http://marketplace.envato.com/api/edge/";
...
function validateLink(url){
   if($.qtEnvImp.expression1.test(url) || $.qtEnvImp.expression2.test(url) || $.qtEnvImp.expression3.test(url) || $.qtEnvImp.expression4.test(url) || $.qtEnvImp.expression5.test(url) || $.qtEnvImp.expression6.test(url) || $.qtEnvImp.expression7.test(url) || $.qtEnvImp.expression8.test(url)) 
	{
		dbg('The link is valid');
		return true;
	}else{
		dbg('The link is not valid!');
		return false;
	}
}
...
function activateStartForm(){
	...
	sendButton.click(function(e){
		...
		if(validateLink(url)){
			//dbg (url);
			url = url.split('http://themeforest.com').join('');
			url = url.split('http://codecanyon.com').join('');
			url = url.split('http://videohive.com').join('');
			url = url.split('http://audiojungle.com').join('');
			url = url.split('http://graphicriver.com').join('');
			url = url.split('http://photodune.com').join('');
			url = url.split('http://3docean.com').join('');
			url = url.split('http://activeden.com').join('');
			
			//dbg("---------->"+url);
			var serviceUrl = window.QTBPIserviceUrl+'&action=ImportItem&Url='+url+'&marketplace='+marketplace+'&BPcheck='+$.BPcheck;
			dbg(serviceUrl);

			if($.qtEnvImp.singleItem.test(url)){
				dbg("importo singolo");
				...
				$.ajax({
				  ...
				  success:function(result){
					...
					var data = [item];	
					dbg(data);				
					generate_list_toimport(data);
				  },
				  error: function(request,state,errors){
				     $("div#qwResponse").html("<strong>Call failed:</strong>"+state+" "+errors);
				     $('#firstLoader').remove();
				     return;
				  }
				});//ajax
			}else{
				var serviceUrl = window.QTBPIserviceUrl+'&action=ImportArchive&Url='+url+'&marketplace='+marketplace+'&BPcheck='+$.BPcheck;
				...
				$.ajax({
				  ...
				  success:function(result){
					...
					if(typeof(obj["collection"]) !== "undefined"){
						var data = obj["collection"];
					}
					if(typeof(obj["new-files-from-user"]) !== "undefined"){
						var data = obj["new-files-from-user"];
					}
			...
			dbg('Non valido');
			$("div#qwResponse").html("<strong>Not an envato marketplace link</strong>");
			$('#firstLoader').remove();
			return;
		}
	})
...
function generate_list_toimport(data){
	...
	var dataToImport=new Array(); 
	//console.log(data);
	if(typeof(data) === "object"){
		for (i in data) { 
			idsToImport[i] = data[i]['id'];
			var theId = data[i]['id'];
			dataToImport[theId] = data[i];
			dbg(theId+" - "+i);
			//dbg("data to import:");
			//dbg(dataToImport[theId]);
		}
		...
		for (id in idsToImport) { 
			output +='<tr id="tabEl'+idsToImport[id]+'"><td><img src="'+data[id]['thumbnail']+'" /></td>\
			<td>\
			<h3>'+data[id]['item']+'</h3>\
			<p><small>Rating: '+data[id]['rating_decimal']+' | Sales: '+data[id]['sales']+' | Uploaded on: '+data[id]['uploaded_on']+' <br> <a href="'+data[id]['url']+'" target="_blank">Visit product page</a></small></p>\
	...
	$("div#qwResponse").append('<table>'+output+'</table>');
	...
	$("a.performImport").click(function(e){
		...
		if($.continuePerfAct == true){// if you click the stop button it goes to false
			if($.statevar[theidtoimport]==0){
				...
				var itemUrl = dataToImport[theidtoimport]['url'];	
				
				/*
				*
				*	Grab the content of the description
				*
				*/
				$.ajax({
			        url: window.QTBPIserviceUrl+'&action=getPageContent',
			        type: "POST",
			        async: false,
			  		data: "url="+itemUrl  	,	
			        success:function(data) {
			            var html = $('.item-description', data).html();
			            dataToImport[theidtoimport]['pagecontent'] =html;
			        },
			        error: function(request,state,errors){
				    	dbg("<strong>Call failed:</strong>"+state+" "+errors);
				  	}
			    }); 

				var thedata =  dataToImport[theidtoimport];


				//console.log(thedata); //corretto
				...
				*/
				...
				$.ajax({
					  ...
					  url: window.QTBPIserviceUrl+'&action=createProduct&releaseid='+theidtoimport+'&BPcheck='+$.BPcheck,
					  type: "POST",
					  data: thedata ,

					  success:function(result){
					  	dbg("cosa vede il php");
					  	dbg(result);
...
function getPageContentByLink(link){
    $.ajax({
        url: window.QTBPIserviceUrl+'&action=getPageContent',
        type: "POST",
        async: false,
        ...
        success:function(data) {
           ...
           // var parser = new DOMParser();
            //$.contents = $($.ajaxData).filter(".item-description").html();
...
/* = Initialization
=============================================================*/
function qtenvatoimporterInitialize(){
...
$(document).ready(function($){
	var initResult = qtenvatoimporterInitialize();
	if(initResult!=true){
		dbg('\n Error initializing');
	}else{
...
$("body").on("click","#qwShowEarnings",function(e){
	e.preventDefault();
	var link = $(this).attr("data-url");
	$("#qwEarningsContainer, #qwSalesContainer").removeClass("qw_hidden");
	$.ajax({
        url: window.QTBPIserviceUrl+'&action=getPageContent',
        type: "POST",
        async: false,
        ...
        success:function(data) {
			...
			jQuery.dataSets = obj;
			var dataArray = jQuery.dataSets["earnings-and-sales-by-month"];
			var salesOld = new Array(); 
			var earningsOld = new Array(); 
			var monthsOld = new Array(); 
			var sales = new Array(); 
			var earnings = new Array(); 
			var months = new Array(); 
			var earningstotal = 0;
			var earningstotalOld = 0;
			var salestotal = 0;
			var salestotalOld = 0;

			for (var i = 0; i < 12; i++) { 
				salesOld[i] = dataArray[i]["sales"];
				earningsOld[i] = dataArray[i]["earnings"];

				salestotalOld = Number(salestotalOld) + Number(salesOld[i]);
				earningstotalOld = Number(earningstotalOld) + Number(earningsOld[i]);

				var monthLabelArray = dataArray[i]["month"].split(" ");
				months[i] = monthLabelArray[1];
			}

			var m = 0;
			for (var i = 12; i < dataArray.length; i++) { 
				sales[m] = dataArray[i]["sales"];	
				earnings[m] = dataArray[i]["earnings"];

				salestotal = Number(salestotal) + Number(sales[m]);
				earningstotal = Number(earningstotal) + Number(earnings[m]);
			...
			*/			
			var lineChartData = {
				...
				datasets : [
					{
						label: "Sales",
						fillColor : "rgba(255,0,186,0.4)",
						strokeColor : "rgba(255,0,186,1)",
						pointColor : "rgba(255,0,186,1)",
						pointStrokeColor : "#fff",
						pointHighlightFill : "#fff",
						pointHighlightStroke : "rgba(255,0,186,1)",
						data : earnings
					}
					,{
						label: "Old Sales",
			...
			var ctx = document.getElementById("qwEarnings").getContext("2d");
			...
			*/
			...
			var ctx = document.getElementById("qwSales").getContext("2d");
			window.myLine = new Chart(ctx).Line(lineChartData, {
				responsive: true
			});
			$("#qwEarningsTotal").html("</p>Earnings past 12 months: "+earningstotalOld+"</p></p>Earnings last 12 months: "+earningstotal+"</p>");
			$("#qwSalesTotal").html("</p>Sales past 12 months: "+salestotalOld+"</p></p>Sales last 12 months: "+salestotal+"</p>");            
        },
        error: function(request,state,errors){
	    	dbg("<strong>Call failed:</strong>"+state+" "+errors);
	  	}
...
$("body").on("click","#qwShowStatement",function(e){
	e.preventDefault();
	var link = $(this).attr("data-url");
	$("#qwStatement").removeClass("qw_hidden");
	$.ajax({
        url: window.QTBPIserviceUrl+'&action=getPageContent',
        type: "POST",
        async: false,
        ...
        success:function(data) {
			...
			jQuery.dataSets = obj;
			//console.log(obj);
			var dataAr = jQuery.dataSets["statement"];
			var table = "";

			for (i in dataAr){
				table = table + "<tr><td>"+dataAr[i]["occured_at"]+"</td><td>"+dataAr[i]["kind"]+"</td><td>"+dataAr[i]["description"]+"</td><td>"+dataAr[i]["amount"]+"</td></tr>";
			}
			$("#qwStatementContent").html(table);
...
$("body").on("click","#qwVerifyPurchase",function(e){
	e.preventDefault();
	var link = $(this).attr("data-url");
	link = link.split("[purchase]").join($("#qwim_purchasecode").val());
	$.ajax({
        url: window.QTBPIserviceUrl+'&action=getPageContent',
        type: "POST",
        async: false,
        ...
        success:function(data) {
            var obj = jQuery.parseJSON(data);
			jQuery.dataSets = obj;
			jQuery.dataAr = jQuery.dataSets["verify-purchase"];

			if(typeof(jQuery.dataAr["buyer"]) !== "undefined"){
				$("#qwAnswer").html("Buyer: <strong>"+jQuery.dataAr["buyer"]+"</strong><br>\
				                    Created At: <strong>"+jQuery.dataAr["created_at"]+"</strong><br>\
				                    Item ID: <strong>"+jQuery.dataAr["item_id"]+"</strong><br>\
				                    Item Name: <strong>"+jQuery.dataAr["item_name"]+"</strong><br>\
				                    Licence: <strong>"+jQuery.dataAr["licence"]+"</strong><br>");
			} else {
				$("#qwAnswer").html("Purchase code invalid");
